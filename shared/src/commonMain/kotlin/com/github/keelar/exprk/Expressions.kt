...
class Expressions {
        ...
        evaluator.addFunction(
            ...
            object : Function() {
                override fun call(arguments: List<BigDecimal>): BigDecimal {
                    ...
                    return arguments.first().roundSignificand(
                        DecimalMode(
                            decimalPrecision = 0,
            ...
            object : Function() {
                override fun call(arguments: List<BigDecimal>): BigDecimal {
                    ...
                    val scale: BigDecimal =
                        if (arguments.size == 2) arguments.last() else BigDecimal.fromInt(0)

                    return value.roundSignificand(
                        DecimalMode(
                            decimalPrecision = scale.longValue(),
                            roundingMode = evaluator.decimalMode.roundingMode,
                        ),
                    )
                }
            },
    ...
    fun setPrecision(precision: Long): Expressions {
        evaluator.decimalMode = DecimalMode(precision, roundingMode)
    ...
    fun evalToString(expression: String): String {
        return try {
            evaluator.eval(parse(expression)).roundSignificand(evaluator.decimalMode)
                .toStringExpanded()
        } catch (e: Throwable) {
